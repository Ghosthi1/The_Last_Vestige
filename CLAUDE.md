...
- **Death handling pattern** — `Health` is pure data; death detection is a dedicated system (`tag_dead_enemies`) that queries entities with `Health`, calls `is_dead()`, and inserts a `Dead` marker component; a second system (`despawn_dead_enemies`) reacts to the marker and despawns; implemented for enemies only so far — colonist death is not yet handled, and no movement/AI queries filter on `Without<Dead>` yet since nothing currently needs to
- **`Dead` marker component** — zero-sized component in `components/combat.rs` alongside `Health`; inserted by `tag_dead_enemies`, consumed by `despawn_dead_enemies`; scoped to enemies only for now — queries that touch it filter `With<Enemy>` explicitly rather than matching any `Health`-having entity
...
- **`Target` component** — lives in `components/combat.rs`; tuple struct `Target(pub Option<Entity>)`; colonist-only; `None` = passive mode (attack nearest enemy in range), `Some(entity)` = spot mode (wait for that specific enemy to enter range); stale entity handling not yet implemented — now that `despawn_dead_enemies` actually removes enemies, a colonist whose spot-target dies stays stuck in spot mode forever (`Target` still holds the dead `Entity`, so it never falls back to passive/nearest-enemy mode); doesn't crash (`colonist_attack`'s `Some(entity)` branch only acts if the entity is still found in that frame's `enemy_snapshot`, which a despawned entity never is), but the colonist effectively goes idle — `Target` should be cleared to `None` when the snapshot lookup fails (TODO, now a live behavioural bug rather than theoretical)
...
- **`tag_dead_enemies` system** — lives in `combat/combat.rs`; queries `(Entity, &Health)` filtered `With<Enemy>, Without<Dead>, Changed<Health>` — the `Changed<Health>` filter means it only runs against enemies that took damage this frame rather than scanning the whole population; inserts `Dead` when `health.is_dead()`; registered `.after(colonist_attack)`
- **`despawn_dead_enemies` system** — lives in `combat/combat.rs`; queries `Entity` filtered `With<Dead>, With<Enemy>`; despawns each; registered `.after(tag_dead_enemies)`; kept as a separate system from tagging (rather than combined) so a future death animation or loot-drop step could be inserted between the two without restructuring either one
